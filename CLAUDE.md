...
| Servicio — el **paquete de prestaciones** que la Prestadora le brinda a su **Cliente Contratante**: una Familia, o un contratante de otro tipo como una Obra Social. Es el concepto **superior**: de un Servicio cuelgan las prestaciones, y de algunas de ellas las Guardias y lo facturable (tabla `servicios`). Un mismo Servicio combina cosas distintas —cuidado por guardias, enfermería, kinesiología, traslados, limpieza del hogar, lo que se acuerde— y puede cubrir a más de un Paciente. Definición completa en `docs/QUE_ES_UN_SERVICIO.md` | usarlo como sinónimo de Guardia o de prestación suelta; usarlo para el pedido de alguien que todavía no contrató (eso es una Solicitud) |
| Prestación — cada cosa concreta que se presta y se cobra dentro de un Servicio, con su precio ya acordado (tabla `prestaciones`). El Servicio es el paquete entero; la prestación es cada cosa del paquete. **Siempre lleva su alcance escrito en dos listas separadas: qué incluye y qué no incluye** — igual que las Tareas de un tipo de Asistente, y por el mismo motivo: la segunda lista es la que evita la confusión | usarla como sinónimo de Servicio ni de Guardia; "renglón de la lista de precios" (eso es el catálogo, no lo vendido); un párrafo único que mezcle lo que incluye con lo que no |
| Cerrar un Servicio — la finalización definitiva de ese Servicio: se deja de prestar y se deja de cobrar, se dan de baja sus prestaciones, se cancelan sus guardias futuras y se avisa a los Asistentes que se quedan sin ese trabajo. Nada se borra. **Sinónimo aceptado, de la jerga del rubro: "levantar el servicio"**, que se usa sobre todo cuando la decisión de terminar la toma la Prestadora | confundirlo con cerrar una Guardia (eso es terminar un turno) ni con dar de baja a un Paciente |
