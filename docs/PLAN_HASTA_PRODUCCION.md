...
- **El número viaja con el chip; la llave se queda en el aparato.** De ahí salen los tres casos: con
  el mismo número en un aparato nuevo no hay ninguna espera, porque lo verificado es el número; con
  un número nuevo en el aparato de siempre tampoco, porque la llave del aparato ya prueba quién es;
  y con las dos cosas nuevas la persona entra igual con su correo y su clave y trabaja el mismo día
  — lo único que espera es que ese número sirva para recuperar la clave.
...
- **Si un mismo número puede quedar verificado para dos personas.** Pasa de verdad: una Familia con
  un solo teléfono en la casa.
- **Si el mensaje de texto entra ahora o queda anotado.** La norma de identidad digital lo bajó a
  vía restringida, permitida a condición de dejar escrito por qué se la usa igual, tener previsto
...
**4.** Lo mismo, en las dos aplicaciones de teléfono. Hoy un Asistente y una Familia no tienen
...
**5.** El mensaje de texto, si va. Es lo único que arranca de cero: hay que contratar un proveedor
...
**6.** Escribir la conexión de ida con el software de facturación de la primera Prestadora, cuando
...
**7.** Terminar de acomodar la pantalla de la Familia al hueco que deja el saldo. El motor ya no
...
**23.** La pantalla del formulario, que es lo único que falta: la base y el motor ya guardan y comprueban los campos de las seis secciones de `docs/PRD_03_Reclutamiento.md`, y el motor entrega las listas de opciones en `GET /api/publico/:prestadora/postulacion-asistente/opciones`. Se dibuja desde la declaración, no a mano. Esperaba el paso anterior.
...
**31. Usted** — La alerta por salida del domicilio, dos decisiones que no puedo tomar yo. Hoy la cuenta se hace con una velocidad media única y distancia en línea recta (`backend/src/utils/llegadaEstimada.js:39-45`), se dispara recién cuando alguien marcó la salida, y mide llegada tarde, no que el Asistente siga en su casa.
...
**43.** Armarlo. La subida del certificado a un depósito de archivos ya quedó resuelta con el
...
**49.** Rotar clave por clave lo que corresponda, decidiéndolo de a una. Si se rota la clave secreta de Supabase, se actualiza en Railway en el mismo acto. Entra acá la clave de servicio de Supabase que estuvo escrita en texto plano en la configuración de permisos de la máquina: los comandos que la llevaban adentro ya se borraron, pero la clave en sí se rota el día de la liberación, no antes.
...
**65. Usted** — El acompañamiento online: ¿prestación más, o guardia sin domicilio? Si es guardia, hay que decidir qué reemplaza al check-in por ubicación.
...
**69. Usted** — La categoría de convenio del Asistente se teclea a mano, y de ella depende su
...
**70. Usted** — ¿Se autoriza construir `careonys.com` según `docs/PRD_01_Sitio_Web.md`? El PRD ya está entero. Hoy `sitio-web/` es una página que dice «En construcción». **El diseño se hace de cero**: del sitio público de Match no viaja nada visual.
...
**73.** Mover los mensajes del sistema a una tabla editable desde afuera, que ya está decidido y
