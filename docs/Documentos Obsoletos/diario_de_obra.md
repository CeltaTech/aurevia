...
|---|---|---|
| 2026-08-07 | **La tercera modalidad deja de llamarse "cooperativa": ahora se llama Subcontratación, y se decide no construirla todavía** (tarea 106, pendiente #115, cerrado). La palabra estuvo tres meses guardada en la base sin definición y nombraba mal lo que guardaba: una cooperativa es una **forma jurídica** —quién es dueño de la empresa y cómo se reparte lo que gana—, y acá se estaba hablando de otra cosa. El Desarrollador la definió el 2026-08-03: la Prestadora toma un servicio de un cliente (por ejemplo una Obra Social) y se lo deriva a otra empresa, que lo cubre con **su propio plantel**. Eso es subcontratar, y la empresa que cubre puede ser una cooperativa, una S.A. o un monotributista: la forma jurídica no cambia nada de cómo funciona el sistema. **Lo primero fue preguntarle a la base viva si el cambio costaba algo:** `prestadora_modalidades` tenía una sola fila, `directa | 2`. Ni una Prestadora con el valor viejo. Por eso se pudo renombrar hoy con un `UPDATE` que no tocó ninguna fila; con clientes adentro habría sido una mudanza de datos. Esto es también lo que permitió saltar la regla 13 de `CLAUDE.md` §7 ("un identificador guardado no se renombra") sin romperla: las dos mitades de la regla apuntaban para el mismo lado, porque el nombre viejo **no** nombraba su función —que es justo lo que la regla pide— y no había dato que proteger. **Lo que no se hizo, a propósito y por escrito: construir la modalidad.** No es una pantalla más; toca cuatro cimientos a la vez —el cliente puede no ser una Familia, la Guardia la cubre gente fuera del plantel (y `asistentes.canales` solo admite `directo` y `marketplace`), el dinero pasa de dos libros a tres porque se le paga a la empresa y no al Asistente, y el control de matrícula y el marcado de presente se quedan sin dueño—. Rehacer media base para una modalidad que ninguna Prestadora usa y que el Panel ni siquiera ofrece no se sostiene, así que queda diferida a post-MVP en el pendiente **#123**, con las **dos decisiones del Desarrollador diferidas junto con ella** en lugar de ocuparle la cabeza hoy con algo que no se va a construir. El valor sigue admitido en las restricciones para que ese día no haga falta otra migración. **El historial no se reescribe:** las migraciones viejas y los dos PRD aprobados conservan el nombre anterior porque son el registro de lo que se decidió ese día (`CLAUDE.md` §10); los PRD llevan un aviso arriba diciendo que el nombre cambió y adónde mirar. Y en `DESIGN_SYSTEM.md` la palabra se queda, porque ahí nombra a una empresa competidora que de verdad es una cooperativa. | `supabase/migrations/20260807180000_la_modalidad_pasa_a_llamarse_subcontratacion.sql` (nuevo: tres `UPDATE` y las tres restricciones `CHECK` rehechas); `panel/src/i18n/translations.js` (la clave y el texto en los 3 idiomas); `panel/src/pages/Dashboard.jsx` (mapa de nombres y contador); `panel/src/context/ModalidadesContext.jsx`, `panel/src/pages/configuracion/Accesos.jsx`, `backend/src/routes/panelCuentas.js`, `backend/src/routes/panelConfiguracion.js`, `supabase/seed.sql` (comentarios); `CLAUDE.md` (§4: entradas **Subcontratación** y **Empresa subcontratada**); `docs/claude_history.md` (entrada nueva del cambio de nombre); `docs/PRD_07_Modalidad_Marketplace.md` y `docs/PRD_08_Dashboard_Modalidades.md` (aviso arriba, texto aprobado intacto); `docs/PENDIENTES.md` (#115 cerrado, #123 nuevo) |
| 2026-08-07 | **El hueco de cobertura por fin avisa: cuando falta poco para una guardia que sigue sin Asistente, el Coordinador se entera** (tarea 68, pendiente #106, cerrado). Era el agujero más grande que había encontrado el inventario del Panel: el proceso que manda los recordatorios le habla siempre al Asistente que tiene la guardia, así que una guardia sin nadie asignado no tenía a quién avisarle y pasaba de largo. Ahora hay un proceso aparte que le habla al Coordinador. **Los dos números —cuántas horas antes avisar y cada cuántas horas repetirlo— los pone cada Prestadora desde Configuración → Avisos**, no están escritos en el código (`CLAUDE.md` §7 regla 1); la cuenta de "¿ya toca repetir?" quedó en un solo archivo que usan los dos procesos que insisten, para no tener la misma cuenta copiada (§7.12). El aviso dice el paciente, el horario, cuánto falta y **en qué anda la búsqueda**: si todavía no se le ofreció a nadie, si hay invitados sin contestar o si todos rechazaron. Probado de punta a punta contra la base local sembrada, con el correo saliendo de verdad a un buzón de pruebas que no sale de la máquina: llega el primer aviso, no se repite antes de tiempo, bajando la anticipación deja de avisar, y atrasando la constancia sale el segundo aviso numerado. De paso quedó anotado el pendiente **#122**: una Prestadora nueva nace sin sus filas de configuración, porque hoy solo se siembran en la migración del día. | `supabase/migrations/20260807120000_aviso_de_guardia_sin_cubrir.sql` (nuevo: tabla `configuracion_aviso_guardia_sin_cubrir` con RLS estricta, y `guardias.aviso_sin_cubrir_at` / `aviso_sin_cubrir_veces`); `backend/src/utils/revisarGuardiasSinCubrir.js` (nuevo); `backend/src/utils/insistencia.js` (nuevo: punto único de "¿ya toca repetir?"); `backend/src/server.js` (el proceso corre cada 5 minutos); `supabase/seed.sql` (configuración de Sandbox + salida de correo al buzón local); `supabase/config.toml` (se abre el puerto de entrada del buzón de pruebas); `panel/src/pages/configuracion/Avisos.jsx` (bloque nuevo); `panel/src/i18n/translations.js` (6 textos × 3 idiomas); `docs/PENDIENTES.md` (#106 cerrado, #122 nuevo) |
| 2026-08-06 | **La base local deja de estar vacía: un archivo de siembra y un solo comando** (tarea 108, pendiente #111, cerrado). Hasta hoy, para mirar una pantalla en el navegador había que escribir filas a mano en la base local, porque tenía las 85 tablas y cero datos: sin Prestadora y sin usuario no se puede ni iniciar sesión. Ahora está `supabase/seed.sql` y se aplica con `npx supabase db reset`. **No hubo que inventar dónde ponerlo:** `supabase/config.toml:66-71` ya declaraba `[db.seed]` apuntando a `./seed.sql`; el archivo simplemente nunca se había escrito. Siembra la Prestadora ficticia **Sandbox** —nombre reservado por `CLAUDE.md` §2, y la única Organización que el Superadmin ve fuera de una sesión de soporte (§5)—, sus dos modalidades activas, tres zonas, diez cuentas (los tres roles del Panel, cuatro Asistentes, tres Familias), tres Pacientes con su Servicio y ocho guardias entre ayer, hoy y mañana, una de ellas **sin cubrir** a propósito. Las fechas son relativas a `CURRENT_DATE`, así que el archivo no envejece. **Dos cosas aprendidas que quedan escritas:** (1) la herramienta que aplica la siembra manda el archivo entero de una vez y **revisa todas las órdenes antes de ejecutar la primera**, así que el camino cómodo —crear una funcioncita arriba y llamarla diez veces abajo— no funciona: en el momento de la revisión esa función todavía no existe. La explicación quedó adentro del propio `seed.sql` para que nadie lo intente de nuevo. (2) El `README.md` del Panel seguía siendo la plantilla en inglés que vino con Vite y no decía una palabra del proyecto; se reescribió entero. **Comprobado en el momento:** el reset corrió limpio y dejó las filas esperadas; después, iniciando sesión de verdad con cinco cuentas y leyendo **con esa sesión**, se ve el aislamiento trabajando —el admin ve 8 guardias y 4 Asistentes, la coordinadora (dos zonas de tres) ve 7 y 3, la Asistente Ana ve solo sus 3, la Familia Gómez ve sus 3 y una sola Asistente—; y con el backend levantado contra esa base, `GET /api/configuracion-publica` devolvió 200 con los datos de Sandbox. Esto es justamente lo que faltaba para poder ejercitar en el navegador el trabajo del pase de guardia (fila de abajo), que quedó sin probar por no tener base local con datos. | `supabase/seed.sql` (nuevo), `panel/README.md` (reescrito de cero), `panel/.env.local` (nuevo, apunta a la base local), `docs/PENDIENTES.md` (#111 cerrado) |
| 2026-08-06 | **El sistema estuvo caído medio día y nadie se enteró: la base de datos de producción estaba pausada** (pendientes #121 y #120, los dos cerrados el mismo día). Apareció tirando del hilo de otra cosa: el respaldo diario venía fallando con `tenant/user not found`, y la explicación fácil —"se rompió una credencial"— resultó falsa. Lo que pasaba era que el proyecto de Supabase `abcpmzfnnhpuiupmrsdi` estaba **pausado**, y un proyecto pausado de Supabase no da "error de servicio": **desaparece del listín de internet**, o sea que su dirección directamente no existe. Comprobado contra tres servidores de nombres independientes. **Tres lecciones que conviene no volver a aprender:** (1) **que las pantallas devuelvan 200 no significa que el sistema ande** — el cascarón HTML lo sirve Cloudflare y no necesita base para nada, así que desde afuera todo parecía normal mientras nadie podía iniciar sesión ni ver un dato; la prueba que sirve es una que obligue a leer la base. (2) **Un respaldo que falla puede estar avisando de algo mucho más grande que el respaldo**; no se lo trata como una tarea de mantenimiento aparte. (3) El razonamiento del Desarrollador —"si el proyecto está pausado no debería funcionar, así que ese no es el que usamos"— era **correcto en la lógica y equivocado en el dato**: sí era el que usábamos. Poner a prueba la premisa en lugar de aceptar la conclusión es lo que destapó la caída. **De paso quedó cerrado el #119** (¿sobra alguno de los dos proyectos de Supabase?): no, hacen falta los dos y no son intercambiables — `abcpmzfnnhpuiupmrsdi` es Careonys y `jpochqufisjjapoudjxj` es el panel comercial de CeltaTech; ya estaba escrito en `docs/CUENTAS.md:192`. **También se tapó un agujero de infraestructura encontrado en el camino:** `deploy-backend.yml` no tenía manera de dispararse a mano, así que cuando GitHub se atrasó media hora en procesar los push, el motor quedó en la versión vieja y no había forma de forzar la publicación sin el token de Railway, que no está en la máquina del Desarrollador. Se le agregó `workflow_dispatch`. **Restaurado el proyecto, todo volvió sin tocar nada más**: las tres pantallas, el motor, la base y el respaldo (corrida `31137221658` en verde, archivo en los dos depósitos). | `.github/workflows/deploy-backend.yml` (se le agrega el disparo manual), `docs/PENDIENTES.md` (#121 nuevo y cerrado, #120 cerrado, #119 cerrado, #118 nuevo) |
| 2026-08-06 | **Cerrar la guardia pasa a ser un botón propio del Asistente, y el candado de continuidad deja de ser un error mudo** (tarea 66, pendiente #113, partes a/b/c — el QR sigue esperando tres decisiones del Desarrollador). Hasta hoy el check-out no existía como acto: sucedía de costado al confirmar el Reporte Diario, así que el Asistente terminaba su turno sin que ninguna pantalla se lo dijera. Ahora son dos cosas separadas: `POST /guardias/:id/checkout` con sus tres controles en orden (sin check-in no hay nada que cerrar; sin reporte cargado no se cierra; si `checkout_bloqueado` está puesto rige el protocolo de continuidad y solo lo libera el Panel con excepción documentada), y del otro lado el botón con su confirmación y su cola sin conexión, igual que el check-in. El candado de continuidad ya estaba entero en la base desde `schema_modulo6_guardias_02.sql` y **ninguna aplicación lo miraba** —la del Asistente incluso lo traía en sus consultas y lo ignoraba—, así que una guardia bloqueada habría terminado en un rechazo crudo de la base mostrado como "ocurrió un error"; ahora se explica con palabras. De paso se sacó la exigencia de GPS del reporte (los dos momentos que dejan constancia de dónde estuvo el Asistente son la llegada y el retiro, y los dos la registran) y se pusieron en uso `hacer_checkout`/`haciendo_checkout`, dos de los 54 textos huérfanos del pendiente #114, con el texto corregido de "Marcar check-out" a "Cerrar la guardia". **Consecuencia anotada como pendiente #117:** ahora una guardia puede quedar sin cerrar, y el informe para la Obra Social solo cuenta las `completada` — tapado del lado del Asistente (vuelve a la pantalla del botón después del reporte, y su lista le marca "Sin cerrar"), falta del lado del Panel. Comprobado: `node --check` del backend, `npm run build` y `npm run lint` de la app del Asistente (limpio salvo tres avisos anteriores a este trabajo). No se pudo ejercitar en el navegador: hace falta el archivo de siembra de la base local (pendiente #111), porque la base de la nube no se usa para pruebas | `backend/src/routes/appAsistentes.js` (endpoint de check-out nuevo, helpers `guardiaDelAsistente()` y `reporteDeLaGuardia()`, el endpoint del reporte ya no cierra la guardia); `pwa-asistentes/src/pages/GuardiaActiva.jsx` (botón, confirmación, aviso de bloqueo, pendiente de envío); `pwa-asistentes/src/pages/ReporteDiario.jsx` (sin GPS, vuelve a la pantalla de la guardia); `pwa-asistentes/src/pages/MisGuardias.jsx` (marca "Sin cerrar"); `pwa-asistentes/src/lib/api.js` (`checkout()` + propagación del `motivo`); `pwa-asistentes/src/lib/sincronizarCola.js` (tercer tipo de ítem); `pwa-asistentes/src/i18n/translations.js` (es-AR/en/pt-BR); `docs/PENDIENTES.md` (#113 a medias, #114 ajustado, #117 nuevo); `docs/claude_history.md` (entrada nueva) |
