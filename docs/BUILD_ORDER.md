...
| 2 | Panel de administración (web desktop, migra a Supabase) | 🟢 Desplegado a producción (2026-07-08). Incluye Módulos 1-5, primer corte de precios/Prestaciones, gestión de usuarios del Panel, Proceso de Incorporación, Certificado de Aptitud, rol Superadmin real, Módulo 7 (Reportes y Alertas, pantallas `Reportes.jsx` y `Alertas.jsx` del Panel) y Módulo 8 (Configuración). El Módulo 6 quedó completo el 2026-08-21 con la última pieza que faltaba, el domicilio temporal del Paciente — ver la fila "Módulo 6 — Guardias" más abajo | Sitio web activo | 1-2 semanas | `PRD_02_Panel_Admin.md` |
...
| 3 | PWA Asistentes (login, guardias, GPS check-in/out, reporte diario + IA Nivel 1) | 🟡 En progreso — primer corte construido, verificado en navegador real y desplegado a producción. De lo que quedó afuera de aquel primer corte a pedido del Desarrollador, dos cosas se construyeron después: las notificaciones al celular (`backend/src/utils/push.js` y `lib/push.js` en las dos aplicaciones) y las alertas tempranas de ausencia (pendiente `#20`). Sigue sin construirse la única que falta: entrar con la huella o la cara del teléfono en vez de la contraseña, que tiene fila propia, la `#154`, en `docs/PENDIENTES.md` | Panel de admin funcionando | 2-3 semanas | `PRD_04_05_App_Servicio.md` |
...
| En paralelo | Módulo 6 — Guardias (series, GPS check-in/out, incidentes de relevo, escalada) | 🟢 Completo — schema aplicado. Parte 1 (Guardias core) construida el 2026-07-10, desplegada y probada en navegador real el 2026-07-11. Parte 2 (Continuidad de guardia) construida, commiteada y probada en navegador real el 2026-07-12. Detección automática de ausencia + alertas tempranas (pendiente #20) construida, schema aplicado y probada en navegador real el 2026-07-13, desplegada. La Parte 3, "piezas de apoyo", eran dos cosas y nada más (`docs/PRD_02_Panel_Admin.md:99`): a quién llamar en una emergencia, y el domicilio temporal de un Paciente. La primera **ya está construida y en producción**: se administra desde Configuración → El Cuidado, con sus rutas propias en el motor y sus textos en los tres idiomas. La segunda también, desde el 2026-08-21: las temporadas en que al Paciente se lo atiende en otro lado se cargan y se dan de baja desde el Panel, y esa dirección es la que ven el Asistente y la Familia y contra la que mide el check-in. Cuál dirección rige cada día se decide en un solo lugar, la función `domicilios_de_pacientes_en` (regla 12). Con eso el Módulo 6 queda cerrado. El envío de mensajes de escalada por WhatsApp está resuelto (ver `docs/PRD_06_WhatsApp_IA.md` y pendiente #9 de `docs/PENDIENTES.md`) | Módulo 4/5 del Panel funcionando | Variable | `PRD_02_Panel_Admin.md` (Módulo 6) |
