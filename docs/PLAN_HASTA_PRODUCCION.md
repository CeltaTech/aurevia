...
**2.** El alta de una Prestadora, que hoy no existe. Las Prestadoras entran sólo por la siembra de la base local y por los guiones de prueba; el Panel las lista y no las crea (`backend/src/routes/panelPrestadoras.js`). Sin ese momento los dos pasos que siguen no tienen dónde engancharse, porque los dos empiezan «al darla de alta». La da de alta el Superadmin desde el Panel, con el nombre, el país y la casilla donde la Prestadora quiere recibir las respuestas. La configuración inicial se siembra sola y eso ya funciona.
...
**4.** El reenvío de las respuestas: una regla de Email Routing por Prestadora, creada en el alta y cortada cuando la Prestadora se va. Cloudflare exige que la Prestadora confirme su dirección de destino con un clic antes de que el reenvío funcione, así que ese estado se muestra.
