...
**Una sola lista, del 1 al 82, en orden.** Se hace el 1, después el 2, y así hasta el final.
...
**1.** Los pesos del cálculo de candidatos (`panel/src/lib/candidatos.js:88`) y los topes de horas
...
**2.** El equipo del Paciente. Se arma solo con quienes tienen turnos habituales con él más la
...
**3.** La ausencia guarda **cuándo se supo**. Hoy pedir vacaciones con un mes y no aparecer esta
mañana son lo mismo. Con tiempo, le llega a la Coordinadora como tarea pendiente; de golpe,
dispara la alarma.

**4.** El expediente del turno vacío. Hoy el incidente sólo nace si había alguien asignado que no
...
**5.** La alarma que no se detiene. El primer escalón ya existe: `configuracion_escalada_coordinador`
...
**6.** La extensión de turno. La Asistente que está adentro sigue a cargo: el sistema se lo
...
**7.** El consentimiento de la Familia para que el Paciente quede solo —quién lo dio, cuándo y por
...
**8.** El cierre honesto del expediente. Hoy sólo se puede cerrar contando **cómo se cubrió**.
...
**9.** La forma de pago y las horas extra. Hoy no existe la modalidad de pago: la decide el código
...
**10.** La Coordinadora registra lo ya arreglado. Si resolvió el reemplazo por teléfono, lo carga
...
**22. Usted** — Dos preguntas de la Solicitud: ¿cómo se le presenta la Asistente nueva a la Familia — aviso sin respuesta, aceptación explícita, o fuera del sistema? ¿Y se construye la vista mapa del plantel por zona, o se saca?
...
**28.** La pantalla del formulario, que es lo único que falta: la base y el motor ya guardan y comprueban los campos de las seis secciones de `docs/PRD_03_Reclutamiento.md`, y el motor entrega las listas de opciones en `GET /api/publico/:prestadora/postulacion-asistente/opciones`. Va junto con la pantalla del Panel donde cada Prestadora carga esas opciones —género, nacionalidad, tipo de registro ante AFIP y los cinco subgrupos de experiencia clínica—, porque hoy `opciones_postulacion` nace vacía y sin ella el formulario no tiene nada que ofrecer. Esperaba el paso anterior.
...
**35. Usted** — Compartir el Certificado de Aptitud: ¿hacia dónde y por qué medio? Hoy se puede ver, con su estado y su fecha. Compartirlo hacia afuera exige decidir a quién se le manda, por qué canal y qué ve quien lo recibe, porque no existe ninguna verificación pública del certificado: sin eso, lo compartido sería una imagen que no prueba nada.

**36. Usted** — La alerta por salida del domicilio, tres decisiones que no puedo tomar yo. Hoy la cuenta se hace con una velocidad media única y distancia en línea recta (`backend/src/utils/llegadaEstimada.js:39-45`), se dispara recién cuando alguien marcó la salida, y mide llegada tarde, no que el Asistente siga en su casa.
...
**39. Usted** — El PRD promete exportar el reporte a PDF en la aplicación de la Familia, y más adelante dice que la Familia no accede al informe. ¿Cuál de las dos vale?
...
**79. Usted** — Las escalas legales: la validación, y los dos valores que el código usa y no
...
**82.** Correr las pruebas y publicar.
