...
/* Qué muestran las dos aplicaciones del teléfono.
   ...
   LA LISTA NO SE ESCRIBE ACÁ. Las dieciocho perillas, con su nombre y su explicación, viven
   en `backend/src/utils/catalogoVisibilidad.js`, que es el único lugar donde se agrega o se
   saca una (CLAUDE.md §7 regla 12). Esta pantalla dibuja lo que le llega: una perilla nueva
   ...
   alguna vez llega una perilla que las traducciones todavía no conocen — es preferible un
   renglón en español antes que un renglón en blanco. Es el mismo camino que la lista de
   avisos de Configuración › Avisos.

   Se guarda con un solo botón y se manda una sola perilla por vez, porque así es la ruta del
...
export function ConfiguracionAplicaciones() {
  const { t } = useLocale();
  const [perillas, setPerillas] = useState([]);
  ...
  const cambiadas = perillas.filter((p) => elegido[p.clave] !== p.visible);
  ...
  async function guardar() {
    ...
    try {
      ...
      for (const perilla of cambiadas) {
        await llamarApi(`/visibilidad-app/${perilla.clave}`, {
          method: 'PATCH',
          body: JSON.stringify({ visible: elegido[perilla.clave] }),
        });
      }
      setGuardado(true);
    } catch (err) {
  ...
  return (
    <div>
      ...
      <EstadoLista
        estado={estado}
        error={error}
        vacio={estado === 'listo' && perillas.length === 0}
        recargar={recargar}
      >
        <div>
          ...
          {APLICACIONES.map((app) => (
            <div key={app}>
              ...
              {perillas
                .filter((perilla) => perilla.app === app)
                .map((perilla) => (
                  <FormField
                    key={perilla.clave}
                    label={t.configuracion[`visibilidad_${perilla.clave}`] || perilla.descripcion}
                    name={perilla.clave}
                    type="checkbox"
                    checked={elegido[perilla.clave] ?? perilla.de_fabrica}
                    ayuda={t.configuracion[`visibilidad_${perilla.clave}_ayuda`] || perilla.ayuda}
                    onChange={(e) => cambiar(perilla.clave, e.target.checked)}
                  />
                ))}
            </div>
          ))}
