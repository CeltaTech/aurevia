...
| Servicio — el arreglo entre la Prestadora y una Familia: a quién se cuida, qué se le hace, cuándo, por cuánto y desde cuándo hasta cuándo. Es el concepto **superior**: de un Servicio cuelgan las Guardias, las prestaciones y lo facturable (tabla `servicios`). Definición completa, con las seis partes y las cinco decisiones de diseño, en `docs/QUE_ES_UN_SERVICIO.md` | usarlo como sinónimo de Guardia o de prestación suelta; usarlo para el pedido de alguien que todavía no contrató (eso es una Solicitud) |
| Prestación — un renglón de un Servicio: una cosa concreta que se presta y se cobra, con su precio ya acordado (tabla `prestaciones`). El Servicio es el arreglo entero; la prestación es cada línea de ese arreglo | usarla como sinónimo de Servicio ni de Guardia; "renglón de la lista de precios" (eso es el catálogo, no lo vendido) |
